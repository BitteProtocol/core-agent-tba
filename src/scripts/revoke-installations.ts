...
import { ENCRYPTION_KEY, WALLET_KEY, XMTP_ENV } from "@/config";
import { createSigner, getEncryptionKeyFromHex } from "@/utils";
...
async function revokeInstallations() {
  try {
    ...
    installationsToRevokeInfo.forEach((inst) => {
      console.log(
        `  ${inst.index}. ${inst.id} (${
          inst.clientTimestampNs || "unknown ts"
        })`,
      );
    });
...
// Run the cleanup
revokeInstallations().catch((error) => {
