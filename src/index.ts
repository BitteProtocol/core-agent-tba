...
import { ENCRYPTION_KEY, NETWORK_ID, WALLET_KEY, XMTP_ENV } from "@/config";
import {
  handleIntentMessage,
  handleTextMessage,
...
import { ActionsCodec } from "./types/actions-content";
import { IntentCodec, type IntentContent } from "./types/intent-content";
...
async function main() {
  ...
  for await (const message of stream) {
    ...
    if (
      ...
      message.contentType?.typeId !== "WalletSendCallsParams"
    ) {
      continue;
    }
    ...
    if (message.contentType.typeId === "text") {
      await handleReaction(message, conversation, senderAddress);
