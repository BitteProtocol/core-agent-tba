...
declare global {
  namespace NodeJS {
    interface ProcessEnv {
      ...
      XMTP_ENV: string;
      BITTE_API_KEY: string;
      BITTE_AGENT_ID: string;
    }
  }
}
  ...
  const messageStream = () => {
    void client.conversations.streamAllMessages((error, message) => {
      ...
      void (async () => {
        ...
        const messageString = typeof message.content === "string" ? message.content : JSON.stringify(message.content);

        try {
          /* Get the AI response */
          const completion = await bitteClient.sendToAgent(BITTE_AGENT_ID, messageString, {
